...
class LSTM(BaseModel):
    """
    LSTM (Long Short-Term Memory) Network for sequence-to-sequence system identification.

    Unlike GRU which has two gates (reset, update), LSTM uses three gates
    (input, forget, output) and a separate cell state, giving it stronger
    ...
    def _build_model(self, input_size: int):
        ...
        class LSTMModel(nn.Module):
            ...
            def forward(self, x, hidden=None):
                ...
                # hidden is a tuple (h_0, c_0) for LSTM
    ...
    def fit(self, u: np.ndarray, y: np.ndarray, verbose: bool = True) -> "LSTM":
        ...
        if verbose:
            epoch_iter = tqdm(epoch_iter, desc="Training LSTM", unit="epoch")
    ...
    def predict_free_run(
        ...
        self.model_.eval()
        hidden = None  # tuple (h, c) for LSTM

        sim_range = range(self.max_lag, n_total)
        if show_progress:
            sim_range = tqdm(sim_range, desc="LSTM Free-run simulation", unit="step")
        ...
        with torch.no_grad():
            for k in sim_range:
                ...
                # Detach hidden state (both h and c)
